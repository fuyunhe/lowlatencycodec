...
)
{
 ...
 ASSERT(att_per_frame == 1 || att_per_frame == 2); /* table based lookup restriction */
   ...
   IF(sub(burst_len, 1) <= 0)
   {
      ...
      {
         L_gr_pow_left[k] = Mpy_32_16(L_oold_tmp, oold_grp_shape_fx[k]); 
         L_gr_pow_right[k] = Mpy_32_16(L_old_tmp, old_grp_shape_fx[k]);   
     ...
     /*   move Xavg   fft scale  to fx domain fx-fft scale*/
        ...
        /* internal transition detection  */
         ...
         { /* pure percentage based transient detection  */
            ...
            tr_dec[k] = s_max(tr_rise[k], tr_decay[k]); 
...
#error trans_burst_ana_fx-- The following implementation is incorrect
   ...
   ELSE /* sub(burst_len, 1) <= 0) */
   {
      ...
      FOR(k = 0; k < Lgw; k++) /* Lgw may be shorter than all bands at 48k */
      {
         ...
         att_val = POW_ATT_TABLES[att_per_frame][s_min(OFF_FRAMES_LIMIT, attDegreeFrames)];   
#else 
...
)
{
    ...
    *num_plocs = 0; 
    ...
    one_peak_flag_mask = (Word16)0xFFFF;  /* all ones mask -> keep  */
    logic16();
    ...
    X[shr_pos(Lprot, 1)] = mult(alpha[s_min(add(fs_idx, LGW8K), LGW48K)], X[shr_pos(Lprot, 1)]); 
    }
#endif
