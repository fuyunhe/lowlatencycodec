                                           ...
                                           Word8 *scratchBuffer)
{
    ...
    IF (sub(nbLostFramesInRow, 1) == 0)
    {
        ...
        ELSE
        {
            ...
            gain_c_32_fx = L_min(gain_c_32_fx, tmp32); 
        }
    }
    ELSE
    {
    ...
    ELSE
    {
        ...
        FOR (i = 0; i < len_hp; i++)
        {
            ...
            exc2_fx[i] = round_fx(L_mac0(tmp32, c1, exc2_fx[i])); 
        }
    }
    ...
    /* update gain */
    *gain_c_fx = L_shl(L_deposit_h(gain_c_16_fx), sub(gain_c_16_fx_exp, 15)); 
    ...
    *Q_syn    = new_Q; 
    ...
    mem_deemph = shr(pcmbufHist_fx[max_len_pcm_plc - 1], -(*Q_syn));
...
static void TDC_normalize_energy_fx(Word16 *gain, Word16 *gain_exp, const Word16 *x, const Word16 lg)
{
    ...
    *gain     = BASOP_Util_Divide1616_Scale((Word16)0x7FFF, tmp16, &e1); 
    *gain_exp = sub(e1, e);                                              
