...
)
{
    ...
    IF (exp0 < 0)
    {
        tmp = shr(-32768, -s_max(exp0, -15)); /* we use negative to correctly represent 1.0 */
        FOR (i = 0; i < lg; i++)
        {
...
)
{
    ...
    IF (exp0 < 0)
    {
        tmp = shr(-32768, -exp0); /* we use negative to correctly represent 1.0 */
        FOR (i = 0; i < lg; i++)
        {
            y[i] = msu_r(0, x[i], tmp); 
        }
...
// ARM
unsigned int isqrt64(long long r)
{
	unsigned int t, q = 0;
	int N = 32;
...
Word32 calculate_correlation(Word32 sum0, Word32 sum1, Word32 sum2)
{
	Word32 corr = 0;	// sum0 <= 0;
	Word16 shift = 15;
	if (sum0 <= 0) {
		return 0;
	}
	UWord32 inv32 = isqrt64((long long)sum1 * sum2);
	if (!inv32) {
		return 32767;
	}
	while (shift && sum0 < (1 << 30)) {
		sum0 <<= 1;
		shift--;
	}
	while (shift && inv32) {
