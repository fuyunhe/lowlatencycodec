...
{
    Dyn_Mem_Deluxe_In(
        Word16  pitch_index, s, *x, ltpf_pitch;
        Word32 *ac32;
        Word16 *ac, *currFrame, *predFrame;
        Word32  ac32_max, cor_max32, sum0, sum1, sum2;
	...
	*mem_in_exp = 0;
#else
	Word16 scale0      = sub(getScaleFactor16_0(mem_in, mem_in_len), 3);
    *mem_in_exp = sub(*mem_in_exp, scale0); 
	Word16 scale1      = sub(getScaleFactor16_0(x, len + 1), 3);
	Word16 x_exp       = RESAMP_EXP - scale1;
	Word16 scale2      = sub(*mem_in_exp, x_exp);
    IF (scale2 > 0)
    {
    ...
    IF (sub(ol_normcorr, 19660) > 0)
    {
...
#if 1
				for (int k = 0; k < 9; k++) {
					sum = L_mac0(sum, ac[pitch_int - ac_min_pitch - 4 + k], ltpf_ac_interp_filt[fr + 3][k]);
				}
#else
				sum = L_mac0(sum, ac[pitch_int - ac_min_pitch - 4], ltpf_ac_interp_filt[fr + 3][0]);
...
#if BTLLC_5MS_MODE
		norm_corr = calculate_correlation(sum0, sum1, sum2);
#else
        scale1   = norm_l(sum1);
        scale2   = norm_l(sum2);
        ...
        {
            norm_corr = s_max(0, round_fx_sat(L_shl_sat(prod, prod_exp))); 
        }
        ELSE
        {
            norm_corr = 32767; 
