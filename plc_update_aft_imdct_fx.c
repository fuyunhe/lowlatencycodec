   ...
   Word16 *ns_cum_alpha, AplcSetup *plcAd)
{
   ...
   if (plcAd)
   {
      ...
      plcAd->PhECU_margin_xfp = s_min(marginNewXlen, marginOldFront);   /* for pHECU winEncalc xfp energy calculations */
      if (frontLen != 0)
      ...
      *ns_cum_alpha = 32767;     /*plc0,  4 , udpate  */
      ...
      if (plcAd)
      {
         ...
         ELSE
         {
             ...
             IF(tmp32 > (0x7D000000>>3) /*1.25*25*/)
             {
                 plcAd->stab_fac = 0;  
             }
             ELSE IF(tmp32 < (0x19003E82>>3) /*0.25*25*/)
             {
